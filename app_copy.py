...
# Tasks Page
    ...
    # Calendar Page
            ...
            if st.session_state.user_role == "Admin":
                query = """
                    SELECT t.id, t.title, t.deadline, t.status, p.name AS project_name,
                        u.username AS assignee_name, t.assigned_to, t.budget
                    FROM tasks t
        ...
        for task in filtered_tasks:
            events.append({
                ...
                "extendedProps": {
                    ...
                    "task_id": task[0],
                    "budget": f"${float(task[7]):,.2f}" if task[7] else "Not set"
                }
            })
        ...
        if calendar_result and "eventClick" in calendar_result:
            ...
            st.session_state.calendar_to_task = {
            ...
            with st.expander(f"Task Details: {event['title']}", expanded=True):
                ...
                st.write(f"**Budget:** {event['extendedProps']['budget']}")
                
                if st.button("View Full Details in Tasks Page"):
                    st.session_state.page = "Tasks"
                    st.rerun()

        # Handle other calendar interactions
        ...
        if calendar_result and "eventDrop" in calendar_result and st.session_state.user_role == "Admin":
