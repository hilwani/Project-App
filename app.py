...
# Tasks Page
        ...
        if 'came_from_calendar' not in st.session_state:
            st.session_state.came_from_calendar = False

        # Handle viewing specific task from calendar
        if st.session_state.viewing_task_id:
            ...
            if task:
                ...
                # JavaScript to scroll to task
                scroll_js = f"""
                <script>
                    function scrollToTask() {{
                        var elements = document.querySelectorAll('[data-testid="stVerticalBlock"]');
        ...
        # Rest of existing tasks page functionality
    ...
    # Calendar Page
        ...
        # Debug: Print session state to understand current state
        st.write("Debug - Current Session State:")
        st.write(f"Viewing Task ID: {st.session_state.get('viewing_task_id')}")
        st.write(f"Came from Calendar: {st.session_state.get('came_from_calendar')}")
        ...
        if 'came_from_calendar' not in st.session_state:
            st.session_state.came_from_calendar = False
        if 'breadcrumbs' not in st.session_state:
            st.session_state.breadcrumbs = ["Calendar"]
        ...
        all_projects = list(set(task[4] for task in all_tasks))
        all_assignees = list(set(task[5] if task[5] else "Unassigned" for task in all_tasks))
        ...
        if calendar_result:
            # Debug: Print full calendar result
            st.write("Debug - Calendar Result:")
            st.write(calendar_result)

            if "eventClick" in calendar_result:
                # Try to extract task ID
                try:
                    clicked_task_id = calendar_result["eventClick"]["event"]["extendedProps"]["task_id"]
                    
                    # Debug: Print clicked task ID
                    st.write(f"Debug - Clicked Task ID: {clicked_task_id}")

                    # Set session state variables to facilitate navigation
                    st.session_state.viewing_task_id = clicked_task_id
                    st.session_state.came_from_calendar = True
                    
                    # Ensure Tasks is in breadcrumbs
                    if "Tasks" not in st.session_state.breadcrumbs:
                        st.session_state.breadcrumbs.append("Tasks")
                    
                    # Change the active page
                    page = "Tasks"
                    st.session_state.page = "Tasks"
                    
                    # Use Streamlit rerun to refresh the page
                    st.rerun()

                except Exception as e:
                    st.error(f"Error processing task click: {e}")
                    st.write(calendar_result)
